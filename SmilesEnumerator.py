...
class SmilesIterator(Iterator):
    ...
    def __init__(self, x, y, smiles_data_generator,
                 batch_size=32, shuffle=False, seed=None, extras=None,
                 dtype=np.float32
                 ):
	...
	if extras is not None:
		e = np.asarray(extras)
		self.extras = np.zeros((extras.shape[0],11,55))
		for i in range(extras.shape[0]):
			for j in range(11):
				self.extras[i][j][0] = e[i][j]
	else:
		self.extras = None
	...
	if self.extras is not None:
		batch_x = np.zeros(tuple([current_batch_size] + [11+self.smiles_data_generator.pad, self.smiles_data_generator._charlen]), dtype=self.dtype)
	    ...
	    if self.extras is None:
		    ...
		    batch_x[i] = np.vstack((x[0], self.extras[j:j+1][0]))
